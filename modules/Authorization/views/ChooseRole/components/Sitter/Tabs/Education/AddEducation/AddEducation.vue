...
const saveForm = async () => {
  ...
  if (!form.id) {
    ...
    await store.createTeacherEducation(
      objectToFormData({
    ...
    await store.updateTeacherEducation(
      objectToFormData({
        id,
        educational_institution,
        faculty,
...
const refresh = async () => {
  ...
  } = await store.getTeacherEducation(route.params.id);
  Object.assign(form, {
    id: route.params.id,
    educational_institution,
    faculty,
...
<template>
  ...
  >
    <div>
      <h1 class="text-dark text-4xl font-bold">Add education</h1>
      ...
      <div class="grid md:grid-cols-2 gap-x-5 mt-[60px]">
        <Input
          ...
          input-class="w-full"
          :required="validate.educational_institution.$errors"
        />
        <Input
          ...
          placeholder="Faculty"
          input-class="w-full"
          :required="validate.faculty.$errors"
        />
        <FormControl
          for="getting-started"
