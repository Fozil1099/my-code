...
onMounted(() => {
  store.getWeekDayList();
  if (route.params.id)
    store.getChildrenScheduleList({
      page: 1,
      isLazy: true,
      page_size: 10,
      children: String(route.params.id),
    });
});
</script>
...
<template>
  ...
  <div class="flex w-full justify-between items-center py-3.5 mt-10">
    ...
    <h3 v-if="store.weekDayList.results.length" class="select-none">
      {{ store.weekDayList.results[selectedDayIndex].name }}
    </h3>
  ...
  <div class="py-7">
    <div class="flex flex-col gap-4">
      ...
      >
        <div
          v-for="(item, index) in store.children.schedule ||
          store.childrenScheduleList.results"
