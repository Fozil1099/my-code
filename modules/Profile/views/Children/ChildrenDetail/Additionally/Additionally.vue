...
const saveAll = async () => {
  const { language, additionally } = authStore.children;
...
onMounted(() => {
  if (!authStore.children.additionally)
    authStore.children.additionally = { hobby: "" };
});
</script>

<template>
  <div class="grid grid-cols-2 gap-5">
    <div class="card px-5 py-2.5">
      <FormControl for="Language" label="Language">
        <div
          ...
          v-modal="'add-lang'"
        >
          <div v-if="authStore.children.language.length">
            {{ authStore.children.language.length }} selected
          </div>
        ...
        <div
          class="flex items-center justify-start flex-wrap gap-3 gap-y-3 mt-2"
        >
          <div
            v-for="item in authStore.children.language"
    ...
    <div class="card px-5 py-2.5">
      <Textarea
        v-if="authStore.children.additionally"
        v-model="authStore.children.additionally.hobby"
        id="Hobby"
        inputClass="w-full"
        label="Hobby"
        placeholder="Hobby"
