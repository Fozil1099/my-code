...
const confirm = () => {
  modal("confirm-mail-input").hide();
  modal("successfully").show();
};
</script>

<template>
  <Modal id="confirm-mail-input" size="max-w-[560px]" footer-hidden>
    <div class="w-full text-center">
      ...
      <template v-if="!isSendCode">
        <div class="flex items-center justify-center py-6">
          <img src="@/assets/images/modal/mail.png" alt="" />
        </div>
        <span class="text-normal">
          Enter your email to get a confirmation code
        </span>
      </template>
      <template v-else>
        <div class="mt-6">
          The code was sent to the mail
          <span class="text-primary"> Pashkatyan2@gmail.com</span>
        </div>
